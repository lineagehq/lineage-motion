...
export type RuntimeObservationCell = Readonly<{ profile: 'normal' | 'reduced'; side: 'source' | 'replay'; runCount: number; browserErrorCount: number; mutatedRunCount: number; mutationCount: number }>;
export const RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION = 'motion.browser-resolved-runtime-observation-diagnostic.v1' as const;
export const RUNTIME_OBSERVATION_SUBSTAGES = ['source-open', 'replay-context', 'replay-page', 'replay-style', 'lifecycle', 'initial-state', 'instrumentation-install', 'start-actions', 'after-start-state', 'actions', 'animation-inventory', 'instrumentation-read', 'samples', 'post-loop-state', 'reload', 'replay-style-reset', 'reset-lifecycle', 'reset-start-actions', 'reset-state', 'close'] as const;
export const RUNTIME_PREPARATION_SUBSTAGES = ['source-open', 'readiness', 'identity-bind', 'inventory', 'replay-build', 'bootstrap-read', 'bootstrap-validate', 'instrumentation-install', 'start-actions', 'actions', 'settle-turn', 'static-runtime-validate', 'animation-count', 'animation-records', 'animated-identity-validate', 'schedule-derive', 'instrumentation-read', 'route-closure', 'ledger-derive', 'ledger-bijection', 'replay-safety', 'result-assemble', 'close', 'cross-run-stability'] as const;
export const IDENTITY_BIND_FAILURE_STEPS = ['derive-node-identities', 'locator-count', 'identity-read', 'binding-marker-write', 'finalize-records'] as const;
export const IDENTITY_DERIVATION_OPERATIONS = ['evaluation-dispatch', 'encoder-initialize', 'element-enumeration', 'sibling-selection', 'child-signature', 'ancestor-record', 'path-serialization', 'provenance-digest', 'stable-input-serialization', 'stable-digest', 'provenance-attribute-write', 'stable-attribute-write'] as const;
export type RuntimeObservationSubstage = typeof RUNTIME_OBSERVATION_SUBSTAGES[number];
export type RuntimePreparationSubstage = typeof RUNTIME_PREPARATION_SUBSTAGES[number];
export type IdentityBindFailureStep = typeof IDENTITY_BIND_FAILURE_STEPS[number];
export type IdentityDerivationOperation = typeof IDENTITY_DERIVATION_OPERATIONS[number];
export type IdentityBindProgress = Readonly<{ bindingCount: number; identityDerivationComplete: boolean; locatorChecksCompleted: number; identityReadsCompleted: number; markerWritesCompleted: number; finalizationComplete: boolean }>;
export type RuntimeObservationSummary = Readonly<
  | { schemaVersion: typeof RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION; variant: 'complete-counts'; completeness: 'complete'; stage: 'observation-cleanliness'; cells: readonly [RuntimeObservationCell, RuntimeObservationCell, RuntimeObservationCell, RuntimeObservationCell] }
  | { schemaVersion: typeof RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION; variant: 'early-failure'; completeness: 'incomplete'; stage: 'observation-execution'; failure: Readonly<{ profile: 'normal' | 'reduced'; side: 'source' | 'replay'; runOrdinal: 1 | 2 | 3; substage: RuntimeObservationSubstage }> }
  | { schemaVersion: typeof RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION; variant: 'preparation-failure'; completeness: 'incomplete'; stage: 'preparation-execution'; failure: Readonly<{ runOrdinal: 1 | 2 | 3; substage: RuntimePreparationSubstage }> }
  | { schemaVersion: typeof RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION; variant: 'identity-bind-failure'; completeness: 'incomplete'; stage: 'preparation-execution'; failure: Readonly<{ runOrdinal: 1 | 2 | 3; substage: 'identity-bind'; step: IdentityBindFailureStep; progress: IdentityBindProgress }> }
  | { schemaVersion: typeof RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION; variant: 'identity-derivation-failure'; completeness: 'incomplete'; stage: 'preparation-execution'; failure: Readonly<{ runOrdinal: 1 | 2 | 3; substage: 'identity-bind'; step: 'derive-node-identities'; operation: IdentityDerivationOperation; evaluationEntered: boolean; enumerationComplete: boolean }> }
>;
...
export function inspectPreprocessorReceipt(value: unknown): boolean {
  if (!baseReceipt(value)) return false;
  const summary = value.runtimeObservationSummary;
  const runtimeRejected = value.admitted === false && value.zeroErrors === false && value.diagnosticCodes.includes('PREPROCESSOR_RUNTIME_ERROR');
  return summary === undefined ? true : runtimeRejected && runtimeSummary(summary);
}
...
function receipt(v: unknown): v is PreprocessorReceipt { return inspectPreprocessorReceipt(v) && digest((v as PreprocessorReceipt).detailedEvidenceSha256); }
function baseReceipt(v: unknown): v is PreprocessorReceipt { const required = ['schemaVersion', 'protocolVersion', 'ownerInputLockVerified', 'browserPinned', 'lockedRoutingComplete', 'closedProfileCovered', 'boundariesComplete', 'profilesComplete', 'threeCleanRuns', 'stableIdentityProven', 'replayEquivalent', 'resetEquivalent', 'zeroReplayRequests', 'zeroErrors', 'deterministicExecution', 'admitted', 'diagnosticCodes']; const optional = ['candidatePackageSha256', 'detailedEvidenceSha256', 'runtimeObservationSummary']; return plain(v) && Object.keys(v).every((key) => [...required, ...optional].includes(key)) && required.every((key) => key in v) && v.schemaVersion === RECEIPT_SCHEMA_VERSION && v.protocolVersion === PREPROCESSOR_VERSION && required.slice(2, -1).every((key) => typeof v[key] === 'boolean') && Array.isArray(v.diagnosticCodes) && v.diagnosticCodes.every((code: unknown) => typeof code === 'string') && (v.candidatePackageSha256 === undefined || digest(v.candidatePackageSha256)) && (v.detailedEvidenceSha256 === undefined || digest(v.detailedEvidenceSha256)); }
function runtimeSummary(v: unknown): v is RuntimeObservationSummary {
  if (!plain(v) || v.schemaVersion !== RUNTIME_OBSERVATION_DIAGNOSTIC_SCHEMA_VERSION) return false;
  if (v.variant === 'identity-derivation-failure') return exact(v, ['schemaVersion', 'variant', 'completeness', 'stage', 'failure']) && v.completeness === 'incomplete' && v.stage === 'preparation-execution' && plain(v.failure) && exact(v.failure, ['runOrdinal', 'substage', 'step', 'operation', 'evaluationEntered', 'enumerationComplete']) && [1, 2, 3].includes(v.failure.runOrdinal) && v.failure.substage === 'identity-bind' && v.failure.step === 'derive-node-identities' && IDENTITY_DERIVATION_OPERATIONS.includes(v.failure.operation) && typeof v.failure.evaluationEntered === 'boolean' && typeof v.failure.enumerationComplete === 'boolean' && (v.failure.operation === 'evaluation-dispatch' ? !v.failure.evaluationEntered && !v.failure.enumerationComplete : v.failure.evaluationEntered) && (['evaluation-dispatch', 'encoder-initialize', 'element-enumeration'].includes(v.failure.operation) ? !v.failure.enumerationComplete : v.failure.enumerationComplete);
  if (v.variant === 'identity-bind-failure') return exact(v, ['schemaVersion', 'variant', 'completeness', 'stage', 'failure']) && v.completeness === 'incomplete' && v.stage === 'preparation-execution' && plain(v.failure) && exact(v.failure, ['runOrdinal', 'substage', 'step', 'progress']) && [1, 2, 3].includes(v.failure.runOrdinal) && v.failure.substage === 'identity-bind' && IDENTITY_BIND_FAILURE_STEPS.includes(v.failure.step) && identityBindProgress(v.failure.step, v.failure.progress);
  if (v.variant === 'preparation-failure') return exact(v, ['schemaVersion', 'variant', 'completeness', 'stage', 'failure']) && v.completeness === 'incomplete' && v.stage === 'preparation-execution' && plain(v.failure) && exact(v.failure, ['runOrdinal', 'substage']) && [1, 2, 3].includes(v.failure.runOrdinal) && RUNTIME_PREPARATION_SUBSTAGES.includes(v.failure.substage);
  if (v.variant === 'early-failure') return exact(v, ['schemaVersion', 'variant', 'completeness', 'stage', 'failure']) && v.completeness === 'incomplete' && v.stage === 'observation-execution' && plain(v.failure) && exact(v.failure, ['profile', 'side', 'runOrdinal', 'substage']) && ['normal', 'reduced'].includes(v.failure.profile) && ['source', 'replay'].includes(v.failure.side) && [1, 2, 3].includes(v.failure.runOrdinal) && RUNTIME_OBSERVATION_SUBSTAGES.includes(v.failure.substage) && (!String(v.failure.substage).startsWith('replay-') || v.failure.side === 'replay');
  if (v.variant !== 'complete-counts' || !exact(v, ['schemaVersion', 'variant', 'completeness', 'stage', 'cells']) || v.completeness !== 'complete' || v.stage !== 'observation-cleanliness' || !Array.isArray(v.cells) || v.cells.length !== 4) return false;
  const order = [['normal', 'source'], ['normal', 'replay'], ['reduced', 'source'], ['reduced', 'replay']]; return v.cells.every((cell: unknown, index: number) => plain(cell) && exact(cell, ['profile', 'side', 'runCount', 'browserErrorCount', 'mutatedRunCount', 'mutationCount']) && cell.profile === order[index]![0] && cell.side === order[index]![1] && ['runCount', 'browserErrorCount', 'mutatedRunCount', 'mutationCount'].every((key) => Number.isSafeInteger(cell[key]) && cell[key] >= 0) && cell.mutatedRunCount <= cell.runCount);
}
function identityBindProgress(step: unknown, v: unknown): v is IdentityBindProgress {
  if (!plain(v) || !exact(v, ['bindingCount', 'identityDerivationComplete', 'locatorChecksCompleted', 'identityReadsCompleted', 'markerWritesCompleted', 'finalizationComplete'])) return false;
  const numbers = [v.bindingCount, v.locatorChecksCompleted, v.identityReadsCompleted, v.markerWritesCompleted];
  if (!numbers.every((item) => Number.isSafeInteger(item)) || v.bindingCount <= 0 || v.markerWritesCompleted < 0 || v.markerWritesCompleted > v.identityReadsCompleted || v.identityReadsCompleted > v.locatorChecksCompleted || v.locatorChecksCompleted > v.bindingCount || typeof v.identityDerivationComplete !== 'boolean' || v.finalizationComplete !== false) return false;
  if (!v.identityDerivationComplete && (v.locatorChecksCompleted !== 0 || v.identityReadsCompleted !== 0 || v.markerWritesCompleted !== 0)) return false;
  if (step === 'derive-node-identities') return !v.identityDerivationComplete && v.locatorChecksCompleted === 0 && v.identityReadsCompleted === 0 && v.markerWritesCompleted === 0;
  if (step === 'locator-count') return v.identityDerivationComplete && v.locatorChecksCompleted < v.bindingCount;
  if (step === 'identity-read') return v.identityDerivationComplete && v.locatorChecksCompleted > v.identityReadsCompleted && v.identityReadsCompleted >= v.markerWritesCompleted;
  if (step === 'binding-marker-write') return v.identityDerivationComplete && v.identityReadsCompleted > v.markerWritesCompleted;
  return step === 'finalize-records' && v.identityDerivationComplete && v.locatorChecksCompleted === v.bindingCount && v.identityReadsCompleted === v.bindingCount && v.markerWritesCompleted === v.bindingCount;
}

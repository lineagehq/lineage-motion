...
import { MotionServiceClient, makeBranchCreateCommand, makeClaimAcquireCommand, makeClaimControlCommand,
  makeTrackCreateCommand, type ActorKind, type CommandResponse, type MotionCommand } from '../../motion-protocol/src/index.ts';
...
export async function runCli(argv: string[], io: Io = { stdout: (value) => process.stdout.write(value),
  ...
  const parsed = parseArgs(argv); if (!parsed) { io.stderr('Usage: motion-cli <track-create|branch-create|claim-acquire|claim-renew|claim-release|claim-revoke> [options]\n'); return 2; }
  try {

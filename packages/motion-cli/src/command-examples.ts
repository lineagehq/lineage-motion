...
export const workflowExamples = {
  sequenceSources: ['sequence-sources'], sequences: ['sequences'],
  sequence: ['sequence', '--sequence-id', '{sequenceId}'],
  sequenceCreate: ['sequence-create', '--sequence-id', 'story', '--name', 'Animation', '--viewport-width', '{widthCssPixels}', '--viewport-height', '{heightCssPixels}', '--claim', 'assembly', '--operation-id', 'create-story', '--expected-revision', '0'],
  sequenceAdd: ['sequence-clip-add', '--sequence-id', '{sequenceId}', '--clip-id', 'opening', '--name', 'Opening', '--index', '0', '--source-document-id', '{documentId}', '--source-revision', '{sourceRevision}', '--claim', 'assembly', '--operation-id', 'add-opening', '--expected-revision', '{revision}'],
  sequenceExport: ['sequence-export', '--sequence-id', '{sequenceId}', '--expected-revision', '{revision}', '--output', '{outputZip}'],
...
export const recoveryHelp = {
  CLI_SEQUENCE_AMBIGUOUS: 'Run sequences and select an exact --sequence-id. Labels are not selectors.',
  CLI_SEQUENCE_CLAIM_REQUIRED: 'Acquire a separate sequence handle with sequence-claim-acquire, or use sequence-create to create and claim atomically.',
  CLI_SEQUENCE_SOURCE_STALE: 'Inspect sequence-sources; choose the intended saved source revision for a new operation. Exact recorded retries preserve their original pins.',
  CLI_SEQUENCE_SOURCE_UNSUPPORTED: 'Inspect sequence-sources for the explicit unsupported reason and viewport; no source was silently omitted.',
  ...
  CLI_CLAIM_PENDING: 'Retry the original command that created this handle: shot-admit for admission, or claim-acquire for acquisition. For sequences, retry sequence-create or sequence-claim-acquire. Keep every original input unchanged, including source, handle, operation ID, scope and expected revision.',
  ...
  CLI_CLAIM_HANDLE_USED: 'For recovery, repeat the exact original shot-admit or claim-acquire that created this handle (sequence-create or sequence-claim-acquire for a sequence). Choose a new handle only for a new admission or acquisition.',

...
import { SqliteProjectStoreBase } from './store-reads.js';

export type FaultPoint = 'after-begin' | 'after-inserts' | 'before-commit' | 'after-commit';
export type HeadRow = { revision: number; json: string; digest: string };
...
export class SqliteProjectStore extends SqliteProjectStoreBase implements ProjectStore {
  ...
  protected reconstructAuthoringState(documentId: string, headRevision: number): AuthoringState {
...
export * from './store-support.js';
import { isAuthoringKind, actorId, affectedIds, failure, stale, forbidden, claimFailure, reducerFailure } from './store-support.js';
